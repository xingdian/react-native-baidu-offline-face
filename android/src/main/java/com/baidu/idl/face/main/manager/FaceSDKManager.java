...
//                        initModel(context, listener);
     ...
     * @param listener
     */
    public void initLicenseOnline(final Context context, final String licenseID, final SdkInitListener listener) {
        PreferencesUtil.initPrefs(context.getApplicationContext());
...
//                    initModel(context, listener);
     ...
     * @param listener
     */
    public void initLicenseBatchLine(final Context context, final String licenseKey, final SdkInitListener listener) {
        PreferencesUtil.initPrefs(context.getApplicationContext());
...
//            // TODO: 模糊，遮挡，光照三个质量检测和姿态角查默认关闭，如果要开启，设置页启动
    ...
    public void onDetectCheck(final byte[] rgbData,
                              ...
                              final Size rgbSize,
                              final Size nirSize,
                              final Size depthSize,
                              final int liveCheckMode,
                              final FaceDetectCallBack faceDetectCallBack) {

        // 【【提取特征+1：N检索：3】
        onDetectCheck(rgbData, nirData, depthData, rgbSize, nirSize, depthSize, liveCheckMode, 3, faceDetectCallBack);
    }
     ...
     * @param nirData            红外YUV 数据流
     * @param depthData          深度depth 数据流
     * @param rgbSize            rgb可见光YUV 数据流-宽高
     * @param nirSize            nir可见光YUV 数据流-宽高
     * @param depthSize          depth可见光YUV 数据流-宽高
        ...
        future = es.submit(new Runnable() {
            ...
            public void run() {
                ...
                if (SingleBaseConfig.getBaseConfig().getType() == 4
                    ...
                    rgbInstance = new BDFaceImageInstance(rgbData, rgbSize.getHeight(),
                            rgbSize.getWidth(), BDFaceSDKCommon.BDFaceImageType.BDFACE_IMAGE_TYPE_RGB,
                            SingleBaseConfig.getBaseConfig().getDetectDirection(),
                            SingleBaseConfig.getBaseConfig().getMirrorRGB());
                } else {
                    rgbInstance = new BDFaceImageInstance(rgbData, rgbSize.getHeight(),
                            rgbSize.getWidth(), BDFaceSDKCommon.BDFaceImageType.BDFACE_IMAGE_TYPE_YUV_NV21,
                            SingleBaseConfig.getBaseConfig().getDetectDirection(),
                            SingleBaseConfig.getBaseConfig().getMirrorRGB());
                }
...
//                LogUtils.e(TIME_TAG, "detect vis time = " + livenessModel.getRgbDetectDuration());
                ...
                if (faceInfos != null && faceInfos.length > 0) {
                    ...
                    onLivenessCheck(rgbInstance, nirData, depthData, rgbSize, nirSize, depthSize, livenessModel.getLandmarks(),
...
//                    LogUtils.e(TIME_TAG, "live rgb time = " + livenessModel.getRgbLivenessDuration());
                ...
                if (liveCheckMode == 3 && nirData != null) {
                    ...
                    // TODO: 用户调整旋转角度和是否镜像，手机和开发版需要动态适配
                    BDFaceImageInstance nirInstance = new BDFaceImageInstance(nirData, nirSize.getHeight(),
                            nirSize.getWidth(), BDFaceSDKCommon.BDFaceImageType.BDFACE_IMAGE_TYPE_YUV_NV21,
                            SingleBaseConfig.getBaseConfig().getDetectDirection(),
...
//            LogUtils.e(TIME_TAG, "feature search time = " + livenessModel.getCheckDuration());
    ...
    public void onAttrDetectCheck(final byte[] rgbData,
        ...
        onDetectCheck(rgbData, nirData, depthData, rgbSize, nirSize, depthSize, liveCheckMode, 1, faceDetectCallBack);
    }
